...
// --- TESTS UNITAIRES getProfile ---------------------------------------------

describe("getProfile - unit test", () => {

  test("ADMIN should get any profile", async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ id_USER: 2, email: "john@test.com", firstname: "John", lastname: "Doe", password: "hashed" })
    const req: any = { user: { id: 1, role: "ADMIN" }, params: { id: "2" } }
    const res: any = { status: vi.fn().mockReturnThis(), json: vi.fn() }
    const next = vi.fn()
    await getProfile(req, res, next)
    expect(res.status).toHaveBeenCalledWith(200)
    expect(res.json).toHaveBeenCalledWith({ id_USER: 2, email: "john@test.com", firstname: "John", lastname: "Doe" })
  })
...
describe("updateProfile - unit test", () => {
  ...
  test("ADMIN can update any profile", async () => {
    ...
    mockPrisma.user.update.mockResolvedValue({ id_USER: 2, email: "new@test.com", firstname: "John", lastname: "Doe", role: "ADMIN" })
    const req: any = { user: { id: 1, role: "ADMIN" }, params: { id: "2" }, body: { email: "new@test.com" } }
    const res: any = { status: vi.fn().mockReturnThis(), json: vi.fn() }
    const next = vi.fn()
  ...
  test("MEMBER must provide correct password", async () => {
    ...
    const req: any = { user: { id: 1, role: "MEMBER" }, params: { id: "1" }, body: { currentPassword: "wrong" } }
    const res: any = {}
  ...
  test("should throw if Prisma fails", async () => {
    ...
    const req: any = { user: { id: 1, role: "ADMIN" }, params: { id: "1" }, body: {} }
    const res: any = {}
